...
TouchScroll.prototype = {
    ...
    resize: function () {
        ...
        this.element = this.$C('div', {
            'class': 'touchscrollelement',
            'style': ';overflow:hidden;width:' + this.scrollWidth + 'px;padding:' + parseInt(barHeight / 2) + 'px ' + parseInt(barWidth / 2) + 'px;position:absolute;top:-' + this.container.scrollTop + 'px;left:-' + this.container.scrollLeft + 'px;'
        });
        this.wrapper = this.$C('div', {
            'class': 'touchscrollwrapper',
            'style': ';overflow:hidden;position:relative;width:100%;height:' + this.clientHeight + 'px;'
        });
        ...
        this.horizontalBar.appendChild(this.horizontalScrollBar);
        this.verticalBar.appendChild(this.verticalScrollBar);
        this.wrapper.appendChild(this.horizontalBar);
        this.wrapper.appendChild(this.verticalBar);
        /*��������ʽ*/
        this.css(this.horizontalBar, {
            'display': this.scrollWidth > this.clientWidth ? 'block' : 'none',
            'width': this.clientWidth + 'px',
            'left': 0,
            'bottom': 0,
            'height': this.cfg.width + 'px'
        });
        this.css(this.verticalBar, {
            'display': this.scrollHeight > this.clientHeight ? 'block' : 'none',
    ...
    slide: function (flag, distance, during, fx, callback) {
        ...
        function run() {
            if (distance && _stime < _during) {
                ...
                _this.timer[flag] = setTimeout(run, 10);
                _this.refresh(flag, true);
            } else {
                _this.during = false;
        ...
        this.during = true;
        clearTimeout(this.timer[flag]);
        run();
    },
    start: function (e) {
        clearTimeout(this.timer[0]);
        clearTimeout(this.timer[1]);//������ڽ��еĶ���
    ...
    move: function (e) {
        ...
        if (typeof this.flag === 'number') {
            var distance,//������Ե��Ҫ�����ĳ���
                ...
                aviliLength;
            distance = 0;
            aviliLength = this[this.property[this.flag][4]] - this[this.property[this.flag][3]];
